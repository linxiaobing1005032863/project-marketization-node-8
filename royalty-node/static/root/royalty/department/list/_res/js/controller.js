...
app.controller('departmentListCtrl',function($scope,departmentSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){
        $scope.delShow = false;
        $state.go('root.royalty.department.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){
        ...
        departmentSer.DepartmentDelete(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.royalty.department.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
    ...
    function activatePage(page) {
        ...
        departmentSer.listDepartment(listData).then(function(response){
            if(response.data.code==0) {
    ...
    departmentSer.countDepartment().then(function(response){
        if(response.data.code==0){
