var app = angular.module('systemcollect', ['ng-pagination','toastr']);
app.controller('systemcollectCtrl',function($scope,systemSer,toastr,$stateParams,$state,$location) {
/*    $scope.a1=function(){
        $scope.visible = true;
    }
    ...
    $scope.cancel = function(){
        $scope.delShow = false;
        $state.go('root.royalty.system.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){
        ...
        systemSer.SystemDelete(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.royalty.system.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
    ...
    $scope.showed=true;
    // 获取项目名称
    systemSer.projectName().then(function(response){
        if(response.data.code == 0){
    ...
    $scope.collect = function(){
        ...
        systemSer.systemCollect(vm.sum).then(function(response){
  ...
  /*  function activatePage(page) {
        ...
        systemSer.listSystem(listData).then(function(response){
            if(response.data.code==0) {
 ...
 $scope.abc = [{unmetAllocationSystem:null,unmetAllocation:null}]
});
